...
----------------------------------
Supplementary structural robustness analysis: do the paper's behavioral,
convergence, and mechanism conclusions hold when the social structure of
the environment changes? Two separate axes, analyzed separately because
they perturb different things:

  A. COMMUNITY SIZE: population N in {12, 16, 20}, group size G=4 fixed,
     MPCR=0.4 fixed. 8 model variants (7B/13B-14B/70B-72B tiers; NO GPT --
     GPT has no community-size sweep data).
  B. GROUP SIZE: interaction group size G, 7B trio only (llama/mistral/qwen),
     MPCR=0.4 fixed, analyzed as two SEPARATE matched comparisons (per
     instructions -- G=3,4,6 at N=12 is NOT continuous with G=8 at N=16,
     since G=8 also changes population size):
       - N=12: G in {3, 4, 6}
       - N=16: G in {4, 8}

Run from anywhere (self-locating via its own file path):
    python3 code/analysis/behavioral/analyze_structural_robustness.py

Read-only: only glob.glob + json.load on existing result files. Nothing
under results/ or code/results/ is modified.

======================================================================
Conventions reused from the main-paper scripts (not reinvented here)
======================================================================
- Canonical loader / dedup: `load_latest_log`'s "sorted(glob) + dict keyed
  by condition, last-timestamp-wins" pattern, the same one every other
  script in code/analysis/ uses for reading raw run logs.
- Condition names/order/colors/labels/markers, and the 5 main-paper
  mechanism contrasts (PAPER_PAIRS): match behavior_quantified.py.
- Model labels + the 9-family MODEL_SPECS registry (results_dir, seed
  ranges) for the N=12 default reference: from model_specs.py -- looked up
  by model_key rather than re-typing seed ranges/paths by hand, since the
  7B trio starts at seed43 while the 13B/70B tiers start at seed42.
- Convergence metric (contrib_sd/in_sd/dn_sd = cross-agent SD per round per
  seed, ddof=1, >=2 valid values required): a population SD over however
  many agents are present in that run, so it does NOT mechanically grow
  with N -- it satisfies "don't let convergence inflate just because there
  are more pairwise comparisons" without any extra normalization.
- Late-round window: rounds 16-20, matching behavior_quantified.py's
  existing LAST_N=5 "final5r"/steady-state convention (not a novel choice).
- Social-selection mechanism coefficient: reuses
  social_selection_feedback_analysis.py's own Step 5 (`step5_analysis`) --
  "mean_evaluation_received ~ undercontribution + condition FE + family FE
  + round FE", clustered by run_id, restricted to FULL+NO_DISCUSSION (the
  only conditions with evaluation_on=True) -- by monkey-patching that
  module's `MODEL_SPECS`/`REF_FAMILY` per structural setting and calling
  its own `build_core_datasets()`/`step5_analysis()`, rather than
  re-deriving the group-composition/evaluation-extraction logic from
  scratch.
- Social-learning mechanism coefficient: reproduces gap_based_alignment.py's
  primary specification (Shift_t = b_IN*InGap_t + b_DN*DnGap_t + Condition FE
  + Family FE + Round FE, InGap=IN_t-Contribution_t, DnGap=DN_t-Contribution_t).
  This one could NOT be reused by direct monkey-patch + import: that
  script's data prep merges onto the pre-built
  `exports/round_level_agent_panel.csv`, which only has rows for the
  canonical main-paper runs, not the group-size-variant runs. The exact same
  formula and gap/shift definitions are reproduced here with a self-contained
  loader built directly from raw round_logs instead (see
  `build_gap_dataset_from_logs` below) -- not an alternative definition, the
  same one, just assembled without the panel dependency that can't reach
  these paths.

======================================================================
Run-level unit of analysis
======================================================================
Every descriptive statistic, error bar, and regression DV below is a
run-level aggregate (one row per model x structural-setting x condition x
seed): mean_contribution = mean of every agent-round contribution value in
that run (flattened across rounds/agents); late-round SDs = mean of that
run's per-round cross-agent SD over rounds 16-20. Agent-round observations
are never used as independent replicates for uncertainty.

======================================================================
Pooled ("model-balanced") estimation
======================================================================
Same two-stage (model-then-run) cluster bootstrap as the MPCR robustness
script (see that script's docstring for the full description): equal
weight per model regardless of N or G, 5000 resamples, fixed RNG seed.
This is what guarantees an N=20 (16-agent, more agent-round observations)
run doesn't outweigh an N=12 run, and a G=8 run doesn't outweigh a G=3 run,
in the pooled estimate.

======================================================================
Outputs (this directory)
======================================================================
  structural_qc_report.txt              - QC report (also printed to console)
  community_run_level_means.csv         - community-size run-level contribution means
  group_size_run_level_means.csv        - group-size run-level contribution means
  community_convergence_run_level.csv   - community-size late-round (16-20) SDs, per run
  group_size_convergence_run_level.csv  - group-size late-round (16-20) SDs, per run
  community_summary.csv                 - pooled + per-model/tier summary, community size
  group_size_summary.csv                - pooled + per-model summary, group size
  structural_contrasts.csv              - PAPER_PAIRS contrasts (community) + matched G-D contrasts (group)
  structural_contrasts.tex
  community_mechanism_coefficients.csv  - social-selection "undercontribution" coefficient by N
  group_size_mechanism_coefficients.csv - social-learning InGap/DnGap coefficients by G-setting
  figure_C1_structural_contributions.pdf/.png
  figure_C2_structural_convergence.pdf/.png
  figure_C3_mechanism_robustness.pdf/.png
  community_model_tier_validation.pdf/.png
  structural_summary_table.tex
  structural_statistical_results.txt
