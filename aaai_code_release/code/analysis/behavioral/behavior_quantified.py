...
----------------------
Tests whether contribution levels differ significantly between
experimental conditions (Q1) and between model families (Q2). Fits OLS
regressions with standard errors clustered by seed, runs a Wald test on
each pairwise contrast, and applies a Bonferroni correction across the
pairs tested within each model/condition.

  Q1 -- within-model: for each model, tests the 5 condition pairs that
        make up the paper's main mechanism comparisons (round included
  ...
  Q2 -- cross-model: for each condition, tests all C(4,2)=6 pairs of
        models against each other (round included as a linear covariate).

Clustering unit: seed (10 seeds per model x condition).
Bonferroni correction: over 5 pairs (Q1) or 6 pairs (Q2).

Sign convention: for a labeled pair "A -> B" (Q1) or "A vs. B" (Q2), the
reported difference is coef(B) - coef(A), so a positive number means the
second-listed condition/model is higher (e.g. "Baseline -> Full: +1.2"
reads as "Full is 1.2 higher than Baseline").

Inputs: raw simulation logs under results/{model}/local/seed{N}/log_*.json
(and, as a fallback for seeds not found there, code/results/{model}/local/
additional_runs/seed{N}/log_*.json).
Outputs (figures/2026-03-22/paper_stats/): q1_all_rounds*.csv and
q2_all_rounds*.csv (the raw contrast numbers), plus q1_contribution*.tex
and q2_contribution*.tex (the same numbers formatted as LaTeX tables).
"""
