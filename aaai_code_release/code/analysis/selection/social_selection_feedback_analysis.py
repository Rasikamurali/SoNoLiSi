...
Simulation details this script's logic depends on: evaluation_on is
perfectly confounded with selection_on across the 4 canonical conditions
(so evaluation-based analyses are restricted to FULL + NO_DISCUSSION, the
only conditions with evaluation_on=True); network weight is updated by TWO
separate mechanical channels each round (evaluation scores AND stated
partner preferences); agents never see their own received evaluation
scores or network weight in any prompt; and "group-seed" is a
form_groups() concept unrelated to the numeric experiment seed.
