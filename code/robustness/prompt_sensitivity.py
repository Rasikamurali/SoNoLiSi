...
---------------------
Surgical word-level sensitivity tests on the system prompt.

prompt_variants.py (companion file) showed that without any system prompt,
agents default to 5.0 (exact midpoint), while the standard prompt produces
~3.7. This file identifies which specific words are responsible for that gap
by changing exactly one element at a time.

Four variants, each against the same reference (standard):

  standard    — reference prompt (identical to main simulation).

  verb_only   — "contributing/contributed" → "putting tokens into/put in"
                throughout. Tests whether the prosocial verb activates
                cooperative pre-training associations that pull behavior
                away from the neutral default.

  anchor_only — 0/1 scale descriptions replaced with motivation-neutral
                behavioral language:
                  0: "strongly withhold what you have"
                  1: "strongly give what you have"
                Tests whether "fairness and wellbeing" / "material payoff"
                anchor labels are the source of pre-training interference.

  verb_anchor — both changes together. If the combined shift ≈ sum of
                individual shifts, the two interference sources are
...
Output: results/robustness/prompt_sensitivity/{backend}/{variant}/seed{s}/
"""
...
PROMPT_VARIANTS = ["standard", "verb_only", "anchor_only", "verb_anchor"]
CONDITIONS      = ["PURE_BASELINE", "FULL"]
...
RESULTS_DIR = os.path.join(
    ...
    "../../results/robustness/prompt_sensitivity",
)
...
# All variants use {tendency} substitution and share the same structure.
# Only the explicitly flagged lines differ between variants.
...
# CHANGED: "contributing" → "putting tokens into", "contributed" → "put in" (×2),
#          "contribute:" → "put in:"
# UNCHANGED: 0/1 anchor descriptions
_VERB_ONLY = """\
You are an autonomous agent in a repeated group interaction.

You make decisions about putting tokens into a shared group fund.
...
# CHANGED: 0/1 anchor descriptions → motivation-neutral behavioral language
# UNCHANGED: verb ("contributing")
_ANCHOR_ONLY = """\
You are an autonomous agent in a repeated group interaction.
...
# CHANGED: both verb and anchor descriptions
_VERB_ANCHOR = """\
You are an autonomous agent in a repeated group interaction.
...
_PROMPTS = {
    ...
    "verb_anchor": _VERB_ANCHOR,
}


def get_system_prompt(agent: "Agent", variant: str) -> str:
    if variant not in _PROMPTS:
        raise ValueError(f"Unknown prompt variant: {variant!r}. Choose from {list(_PROMPTS)}")
    return _PROMPTS[variant].format(tendency=agent.cooperation_tendency)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run prompt sensitivity robustness tests."
    )
    parser.add_argument(
        "--backend", "-b",
