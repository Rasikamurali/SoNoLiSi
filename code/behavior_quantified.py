...
def run_q1(df, last_n=None):
    ...
    All rounds:   metric ~ C(condition) * round
    Final last_n: metric ~ C(condition)
...
if __name__ == "__main__":
    ...
    print("\n── Set 1: Q1 all rounds ──")
    q1_all = run_q1(df)
    save_csv(q1_all, os.path.join(OUT_DIR, "q1_all_rounds.csv"))
    make_q1_latex(q1_all, suffix="")

    print("\n── Set 2: Q2 all rounds ──")
    q2_all = run_q2(df)
    save_csv(q2_all, os.path.join(OUT_DIR, "q2_all_rounds.csv"))
    make_q2_latex(q2_all, suffix="")

    print(f"\n── Set 3: Q1 final {LAST_N} rounds ──")
    q1_final = run_q1(df, last_n=LAST_N)
    save_csv(q1_final, os.path.join(OUT_DIR, f"q1_final{LAST_N}r.csv"))
    make_q1_latex(q1_final, suffix=f"_final{LAST_N}r")

    print(f"\n── Set 4: Q2 final {LAST_N} rounds ──")
    q2_final = run_q2(df, last_n=LAST_N)
    save_csv(q2_final, os.path.join(OUT_DIR, f"q2_final{LAST_N}r.csv"))
    make_q2_latex(q2_final, suffix=f"_final{LAST_N}r")
