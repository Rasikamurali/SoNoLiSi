...
Three metrics computed per seed per round:
  1. alignment    — Spearman r(incoming_weight, contribution) across agents
                    Does the network learn to weight cooperators more?
  2. gini         — Gini coefficient of incoming-weight distribution
                    Does weight become more concentrated (unequal) over time?
  3. top_share    — fraction of total incoming weight held by top-25% contributors
...
METRIC_LABELS = {
    "alignment": r"Spearman $r$(weight, contribution)",
...
def compute_round_metrics(r):
    ...
    # 1. Spearman alignment
    if len(set(iw_vals)) < 2 or len(set(con_vals)) < 2:
        alignment = np.nan
    else:
        alignment, _ = spearmanr(iw_vals, con_vals)

    # 2. Gini of incoming weight
    g = gini(iw_vals)

    # 3. Top-quartile contributor weight share
