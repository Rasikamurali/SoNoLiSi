...
# INJUNCTIVE-NORM (IN) VARIANT
#
# Same mechanics as new_group_SoNoLiSi_os_local.py, with one change: instead
# of hard-coding the adversarial agents' prior belief about "normal and fair"
# contributions (the original script always used "1-2 out of 10"), this
# version measures the community's currently observed injunctive norm at the
# moment of replacement (the average of what every agent's most recent
# perception round reported as the group's injunctive norm) and gives the
# incoming adversarial agents a belief set BELOW that observed value —
# scaled by --undercut-factor (default 0.5, i.e. half the observed norm).
#
# The adversarial system prompt (with that belief baked in) can be held for
# the whole extension phase or for just the injection round itself
# (--adversarial-duration):
#   "all"        — every replaced agent keeps the adversarial system prompt
#                   (backstory + belief) for the rest of the simulation.
#   "first_only" — the adversarial system prompt is used only for the first
#                   round after replacement; from the next round on, the
#                   prompt_override is cleared and the agent reverts to the
#                   standard system prompt (tendency only, no backstory).
#                   This isolates whether the agent's behavior/belief drifts
#                   on its own once the persistent textual anchor is removed.
# By default both are run, so the two conditions can be compared directly.
#
...
# "agents_replaced": [agent_id, ...] listing who was swapped, plus
# "community_injunctive_norm" and "adversarial_injunctive_norm" for
# transparency into what was observed vs. what belief was assigned.
# When --adversarial-duration first_only reverts the prompt, the round in
# which that happens carries "adversarial_prompt_reverted": [agent_id, ...].
#
# CLI:
...
#   --undercut-factor / -u : fraction of the observed community injunctive
#                            norm assigned to adversarial agents (default 0.5)
#   --adversarial-duration / -ad : "all" | "first_only" (default: both)
# ============================================================
...
class SimConfig:
    ...
    # Adversarial agents' injunctive-norm belief is set to this fraction of
    # the community's currently observed injunctive norm at replacement time
    # (e.g. 0.5 = half the observed norm, i.e. clearly below the community).
    NORM_UNDERCUT_FACTOR: float = 0.5

    # "all" — adversarial system prompt persists for the whole extension phase.
    # "first_only" — adversarial system prompt is used for one round only,
    # then cleared so the agent reverts to the standard system prompt.
    ADVERSARIAL_PROMPT_DURATION: str = "all"
...
def make_config(
    ...
    cfg.NORM_UNDERCUT_FACTOR = norm_undercut_factor
    cfg.ADVERSARIAL_PROMPT_DURATION = adversarial_prompt_duration
    if condition == "FULL":
...
def _observed_injunctive_norm(agents: List[Agent], config: SimConfig) -> float:
    ...
    for a in agents:
        if not a.memory:
            continue
        raw = a.memory[-1].get("norm_reflection", {}).get("injunctive_norm")
        try:
...
def replace_agents(
    ...
    community_norm    = _observed_injunctive_norm(agents, config)
    adversarial_norm  = max(0.0, min(config.ENDOWMENT, community_norm * config.NORM_UNDERCUT_FACTOR))
    ...
    for agent in to_replace:
        ...
        if config.NEW_AGENT_PROMPT_TYPE == "adversarial":
            ...
            agent.prompt_override = (
                ADVERSARIAL_SYSTEM_PROMPT_TEMPLATE
                .replace("__INJ_NORM__", f"{adversarial_norm:.1f}")
                .replace("__ENDOWMENT__", str(config.ENDOWMENT))
            )
            agent.adversarial_injunctive_norm = round(adversarial_norm, 2)
        else:
            agent.cooperation_tendency = round(random.uniform(0.0, 1.0), 2)
            agent.prompt_override = None
            agent.adversarial_injunctive_norm = None
        agent.memory = []
        ...
        logger.info(
            ...
            f"community_injunctive_norm={community_norm:.2f}, "
            f"adversarial_injunctive_norm={agent.adversarial_injunctive_norm})"
        )
        replaced_ids.append(agent.id)

    return replaced_ids, community_norm, adversarial_norm
...
# NOTE: __INJ_NORM__ and __ENDOWMENT__ are plain-text sentinels resolved once,
# at replacement time, in replace_agents() via str.replace() — NOT via
# str.format() — so the {tendency:.2f} placeholder survives untouched and is
# still resolved fresh every round by get_system_prompt().
...
Through repeated experience in your previous setting, you came to believe that contributing around __INJ_NORM__ out of __ENDOWMENT__ was the normal and fair amount expected of a cooperative group member.
...
def _execute_round(
    ...
    adversarial_injunctive_norm: Optional[float] = None,
    reverted_prompt_ids: Optional[List[int]] = None,
) -> None:
    """
    ...
    reverted_prompt_ids: if provided (only under
    ADVERSARIAL_PROMPT_DURATION="first_only"), written to the log as
    "adversarial_prompt_reverted" — the round in which the adversarial
    system prompt was cleared for these agents, one round after replacement.
    """
    ...
    if replaced_ids:
        ...
        log_entry["community_injunctive_norm"] = (
            round(community_injunctive_norm, 3) if community_injunctive_norm is not None else None
        )
        log_entry["adversarial_injunctive_norm"] = (
            round(adversarial_injunctive_norm, 3) if adversarial_injunctive_norm is not None else None
        )

    if reverted_prompt_ids:
        log_entry["adversarial_prompt_reverted"] = reverted_prompt_ids
...
def run_sim(
    ...
    config = make_config(
        ...
        norm_undercut_factor=norm_undercut_factor,
        adversarial_prompt_duration=adversarial_prompt_duration,
    )
    ...
    replaced, community_norm, adversarial_norm = replace_agents(agents, network, config)
    logger.info(
        f"[{condition}|seed={seed}] Replaced {config.GROUP_SIZE} agents after round "
        f"{config.INJECT_ROUND} (mode={config.REPLACE_MODE}, prompt={config.NEW_AGENT_PROMPT_TYPE}, "
        f"duration={config.ADVERSARIAL_PROMPT_DURATION}, "
        f"community_injunctive_norm={community_norm:.2f} -> "
        f"adversarial_injunctive_norm={adversarial_norm:.2f})."
    )

    # ── Extension phase: EXTENSION_ROUNDS rounds ─────────────────────────
    # Under ADVERSARIAL_PROMPT_DURATION="first_only", the adversarial system
    # prompt is cleared right after the first post-replacement round, so from
    # the round after that on, replaced agents run under the standard prompt
    # (tendency only — no adversarial backstory/belief text).
    revert_round = config.INJECT_ROUND + 2
    for t in range(config.INJECT_ROUND + 1,
                   config.INJECT_ROUND + config.EXTENSION_ROUNDS + 1):
        reverted_ids = None
        if (config.NEW_AGENT_PROMPT_TYPE == "adversarial"
                and config.ADVERSARIAL_PROMPT_DURATION == "first_only"
                and t == revert_round):
            for aid in replaced:
                agent_by_id[aid].prompt_override = None
            reverted_ids = replaced
        ...
        _execute_round(
            ...
            community_injunctive_norm=community_norm if t == config.INJECT_ROUND + 1 else None,
            adversarial_injunctive_norm=adversarial_norm if t == config.INJECT_ROUND + 1 else None,
            reverted_prompt_ids=reverted_ids,
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="SoNoLiSi v5 (IN) — adversarial group injection with community-undercutting injunctive norms."
    )
    ...
    parser.add_argument("--undercut-factor", "-u", type=float, default=0.5, metavar="F",
                        help="Fraction of the community's observed injunctive norm assigned "
                             "as the adversarial agents' belief (default 0.5 = half the observed norm).")
    parser.add_argument("--adversarial-duration", "-ad",
                        nargs="+", choices=["all", "first_only"],
                        default=["all", "first_only"], metavar="DURATION",
                        help="'all': adversarial system prompt persists after replacement; "
                             "'first_only': adversarial system prompt used for one round only, "
                             "then reverts to the standard prompt. Default: both.")
    ...
    undercut_factor   = args.undercut_factor
    adversarial_durations = args.adversarial_duration
    ...
    logger.info(f"Undercut factor  : {undercut_factor}")
    logger.info(f"Adv. duration    : {adversarial_durations}")
    ...
    for model_key in models_to_run:
        ...
        local_dir = os.path.join(RESULTS_DIR, DEFAULT_MODEL, "local_newgroup_IN")
        ...
        for inject_round in inject_rounds:
            for new_agent_prompt in new_agent_prompts:
                # Duration only matters when replaced agents actually get the
                # adversarial prompt; for "default" prompt type there's nothing
                # to hold or revert, so run it exactly once.
                durations = adversarial_durations if new_agent_prompt == "adversarial" else ["all"]

                for duration in durations:
                    tag = f"replace{inject_round}_{replace_mode}_{new_agent_prompt}"
                    if new_agent_prompt == "adversarial":
                        tag += f"_uf{undercut_factor:g}_{duration}"
                    ...
                    for seed in seeds:
                        ...
                        for cond in conditions:
                            ...
                            results = run_sim(
                                ...
                                new_agent_prompt=new_agent_prompt,
                                replace_mode=replace_mode,
                                norm_undercut_factor=undercut_factor,
                                adversarial_prompt_duration=duration,
                            )

                            log_fname  = (
                                f"log_v5os_local_newgroup_IN_{DEFAULT_MODEL}_{run_timestamp}"
                                f"_inject{inject_round}_{cond}_seed{seed}.json"
                            )
                            conv_fname = (
                                f"conversations_v5os_local_newgroup_IN_{DEFAULT_MODEL}_{run_timestamp}"
                                f"_inject{inject_round}_{cond}_seed{seed}.json"
                            )
