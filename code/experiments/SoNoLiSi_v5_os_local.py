...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Run SoNoLiSi v5 open-source local simulation."
    )
    parser.add_argument(
        "--model", "-m",
        nargs="+",
    ...
    parser.add_argument(
        ...
        default=10,
        metavar="N",
        help="Number of seeds to run, taken from the start of the default list. Default: 11.",
    )
    parser.add_argument(
        "--device", "-d",
        type=int,
    ...
    for model_key in models_to_run:
        ...
        local_dir = os.path.join(RESULTS_DIR, DEFAULT_MODEL, "local")
        ...
        for seed in seeds:
            ...
            for cond in conditions:
                logger.info(f"\n{'=' * 50}\nRunning: {cond} | seed={seed} | model={DEFAULT_MODEL} | N={n_agents} G={group_size}\n{'=' * 50}")
                results = run_sim(condition=cond, seed=seed,
                                  n_agents=n_agents, group_size=group_size)

                logs_path = os.path.join(seed_dir, f"log_v5os_local_{DEFAULT_MODEL}_{run_timestamp}_{cond}_seed{seed}.json")
                with open(logs_path, "w") as f:
                    json.dump(results.to_dict(), f, indent=2, default=str)
                logger.info(f"Saved logs → {logs_path}")

                conv_path = os.path.join(seed_dir, f"conversations_v5os_local_{DEFAULT_MODEL}_{run_timestamp}_{cond}_seed{seed}.json")
