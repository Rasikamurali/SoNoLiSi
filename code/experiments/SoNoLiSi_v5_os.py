# ============================================================
# LLM-AGENT NORM EMERGENCE — v5 OPEN-SOURCE BACKEND
#
# Identical simulation logic to SoNoLiSi_v5.py (cooperation_tendency,
# directed network, sticky reputation, tendency update) but uses local
# HuggingFace models instead of the OpenAI API.
#
# Models are loaded once and shared across all agents.
# Agent-level model assignment is supported via MODEL_ASSIGNMENTS.
# ============================================================
...
def update_tendencies_from_norm(
    ...
    Pull each agent's cooperation_tendency toward their perceived injunctive norm.
      ...
      target    = injunctive_norm / ENDOWMENT
...
def get_excluded_agents(
    ...
    for this round but remain in the network.
    """
    excluded = set()
...
def build_perception_prompt(
    ...
    return (
        ...
        f"Output JSON with EXACT keys:\n"
        f'{{\n'
        f'  "injunctive_norm": <float 0-{config.ENDOWMENT}, the contribution level you think agents in this group believe people SHOULD contribute, regardless of what they actually did — make your best estimate based on observed behavior and group norms>,\n'
...
def run_sim(condition: str, seed: int) -> SimResults:
    ...
    for t in range(1, config.ROUNDS + 1):
        ...
        # 5. EVALUATION — skipped when evaluation_on is False
        evaluations: Dict[Tuple[int, int], float] = {}
        ...
        # 6. PERCEPTION — also updates network via stated partner preferences
        perceptions: Dict[int, dict] = {}
...
if __name__ == "__main__":
    ...
    for seed in seeds:
        for cond in conditions:
            ...
            logs_path = os.path.join(RESULTS_DIR, f"log_v5os_{DEFAULT_MODEL}_{run_timestamp}_{cond}_seed{seed}.json")
            with open(logs_path, "w") as f:
                json.dump(results.to_dict(), f, indent=2, default=str)
            logger.info(f"Saved logs → {logs_path}")

            conv_path = os.path.join(RESULTS_DIR, f"conversations_v5os_{DEFAULT_MODEL}_{run_timestamp}_{cond}_seed{seed}.json")
