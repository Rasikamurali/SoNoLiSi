...
#
# Identical to SoNoLiSi_v5_os_local.py except that the public-
# goods multiplier is no longer fixed: it is computed from a
# constant Marginal Per-Capita Return (MCPR) so that the
# incentive to cooperate is held constant across group sizes.
#
#   multiplier = MCPR × group_size   (default MCPR = 0.4)
#
# With MCPR = 0.4 and group_size = 4  → multiplier = 1.6  (same
# as the baseline), but e.g. group_size = 8 → multiplier = 3.2.
#
# Because the multiplier now scales with the actual group, the
# per-capita return from a single token contributed is always
# MCPR regardless of how large the group is, allowing clean
# comparisons of group-size effects on norm emergence without
# confounding the incentive structure.
#
# The group multiplier is recomputed per round per group using
# the real group size (len(group_ids)), so it also handles the
# slightly uneven groups that can arise from remainder agents.
#
...
class SimConfig:
    ...
    GROUP_SIZE: int = 4
    ENDOWMENT: int = 10
    # MCPR (Marginal Per-Capita Return): per-token return to each group member.
    # The actual multiplier applied to the group fund is MCPR × group_size,
    # keeping the incentive to cooperate constant across group sizes.
    MCPR: float = 0.4
...
def build_decision_prompt(
    ...
    # Multiplier = MCPR × actual group size, keeping per-token return constant
    multiplier = config.MCPR * len(group_ids)
...
def run_sim(condition: str, seed: int,
    ...
    for t in range(1, config.ROUNDS + 1):
        ...
        # ----------------------------------------------------------
        # 4. PAYOFFS
        # Multiplier = MCPR × actual group size, recomputed per group
        # so that uneven groups (from remainder agents) also get the
        # correct MCPR-normalised incentive.
        # ----------------------------------------------------------
        ...
        round_logs.append({
            ...
            "mcpr": config.MCPR,
            "group_multipliers": {
                str(g_idx): round(config.MCPR * len(group_ids), 4)
                for g_idx, group_ids in enumerate(groups)
            },
...
if __name__ == "__main__":
    ALL_SEEDS      = list(range(42, 52))   # seeds 42–51 (10 seeds)
    ALL_CONDITIONS = ["FULL", "NO_DISCUSSION", "NO_SELECTION", "BASELINE", "PURE_BASELINE"]
    ALL_MODELS     = list(AVAILABLE_MODELS.keys())

    parser = argparse.ArgumentParser(
        description="Run SoNoLiSi v5 open-source local simulation with MCPR-normalised multiplier."
    )
    parser.add_argument(
        "--model", "-m",
        nargs="+",
    ...
    parser.add_argument(
        ...
        default=0.4,
        metavar="MCPR",
        help="Marginal per-capita return (default: 0.4). Multiplier = MCPR × group_size.",
    )
    ...
    group_tag = f"N{n_agents}_G{group_size}_MCPR{mcpr}"
    ...
    for model_key in models_to_run:
        ...
        model_dir = os.path.join(RESULTS_DIR, DEFAULT_MODEL, "local_groupsizevary", group_tag)
        ...
        for seed in seeds:
            ...
            for cond in conditions:
                logger.info(
                    ...
                    f"| N={n_agents} G={group_size} MCPR={mcpr}\n{'=' * 50}"
                )
                results = run_sim(
                ...
                logs_path = os.path.join(
                    seed_dir,
                    f"log_v5os_local_groupsizevary_{DEFAULT_MODEL}_{run_timestamp}_{cond}_seed{seed}.json",
                )
                ...
                conv_path = os.path.join(
                    seed_dir,
                    f"conversations_v5os_local_groupsizevary_{DEFAULT_MODEL}_{run_timestamp}_{cond}_seed{seed}.json",
                )
