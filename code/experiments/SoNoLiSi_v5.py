...
def make_config(condition: str, n_agents: int = 12, group_size: int = 4) -> SimConfig:
    cfg = SimConfig()
    cfg.N          = n_agents
...
def run_sim(condition: str, seed: int,
    ...
    config = make_config(condition, n_agents=n_agents, group_size=group_size)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run SoNoLiSi v5 GPT simulation.")
    parser.add_argument("--conditions", "-c", nargs="+",
                        choices=ALL_CONDITIONS, default=ALL_CONDITIONS, metavar="COND",
    ...
    parser.add_argument("--n-agents", type=int, default=12, metavar="N",
                        help="Total agents in the simulation (default: 12).")
    ...
    parser.add_argument("--variant", "-v", default="global",
                        ...
                        help="Simulation variant — global (pre-group discussion) or local (post-group). Default: global.")
    ...
    base_dir  = os.path.join(os.path.dirname(os.path.abspath(__file__)), "../results/gpt")
    out_base  = os.path.join(base_dir, variant, group_tag) if group_tag \
                else os.path.join(base_dir, variant)

    logger.info(f"Conditions : {conditions}")
    logger.info(f"Seeds      : {seeds}")
    logger.info(f"N agents   : {n_agents}  |  Group size: {group_size}")
    ...
    for seed in seeds:
        ...
        for cond in conditions:
            logger.info(f"\n{'=' * 50}")
            logger.info(f"Running: {cond} | seed={seed} | N={n_agents} G={group_size}")
            logger.info(f"{'=' * 50}")
            ...
            logger.info(f"Saved {len(results.conversation_logs)} conversations → {conv_path}")
