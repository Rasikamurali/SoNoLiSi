...
if __name__ == "__main__":
    ...
    for seed in seeds:
        ...
        for cond in conditions:
            ...
            logs_path = os.path.join(seed_dir, f"log_v5local_{run_timestamp}_{cond}_seed{seed}.json")
            with open(logs_path, "w") as f:
                json.dump(results.to_dict(), f, indent=2, default=str)
            logger.info(f"Saved logs → {logs_path}")

            conv_path = os.path.join(seed_dir, f"conversations_v5local_{run_timestamp}_{cond}_seed{seed}.json")
