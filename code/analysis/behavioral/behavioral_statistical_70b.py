"""
behavioral_statistical_70b.py
------------------------------
Same five behavioral statistical tests as behavioral_statistical.py,
applied to the 70B/72B model family:
  Llama-70B, Qwen-72B

Data source: code/results/{model}/local/seed*/log_*.json  (N=12, G=4, MCPR=0.4)
Outputs  →  figures/statistical_tests/behavioral_70b/

Note: with only 2 families the omnibus LRT has df=(5-1)*(2-1)=4.
"""
...
MODELS_70B = ["llama_70b", "qwen_72b"]
LABELS_70B = {
    "llama_70b": "Llama-70B",
    "qwen_72b":  "Qwen-72B",
}

# 70B data lives under code/results, not results/
RESULTS_70B  = "/data3/rasimura/social-norm-evo/code/results"
FIG_ROOT_70B = "/data3/rasimura/social-norm-evo/figures/statistical_tests/behavioral_70b"

# Override module-level globals
_bs.FAMILIES = ["Llama-70B", "Qwen-72B"]
_bs.REF_FAM  = "Llama-70B"
...
def _load_logs():
    best = {}
    for model in MODELS_70B:
        pattern = os.path.join(RESULTS_70B, model, "local", "seed*", "log_*.json")
...
def build_dataframe() -> pd.DataFrame:
    ...
    for (model, _, _), (s, d) in _load_logs().items():
        for r in d["round_logs"]:
            rows.append({
                "family":    LABELS_70B[model],
                "seed":      s,
                "condition": d["condition"],
                "round":     r["round"],
                "contrib":   float(np.mean(list(r["contributions"].values()))),
                "run_id":    f"{LABELS_70B[model]}_s{s}_{d['condition']}",
            })
...
def main():
    os.makedirs(FIG_ROOT_70B, exist_ok=True)

    print("=" * 70)
    print("BEHAVIORAL STATISTICAL TESTS — 70B/72B MODELS")
    print("Llama-70B | Qwen-72B")
    print("=" * 70)
    ...
    means_df.to_csv(os.path.join(FIG_ROOT_70B, "level_condition_means.csv"), index=False)
    level_cdf.to_csv(os.path.join(FIG_ROOT_70B, "level_contrasts.csv"), index=False)
    _bs.write_level_tex(means_df, level_cdf, os.path.join(FIG_ROOT_70B, "level_contrasts.tex"))
    ...
    slope_df.to_csv(os.path.join(FIG_ROOT_70B, "slope_condition_estimates.csv"), index=False)
    slope_cdf.to_csv(os.path.join(FIG_ROOT_70B, "slope_contrasts.csv"), index=False)
    _bs.write_slope_tex(slope_df, slope_cdf, os.path.join(FIG_ROOT_70B, "slope_contrasts.tex"))

    # ── Test 3: Omnibus LRT ──────────────────────────────────────────────────
    print("\n" + "=" * 70)
    print("TEST 3 — CROSS-MODEL CONSISTENCY (OMNIBUS LRT)")
    print(f"  Note: 2 families → df = (5-1)*(2-1) = 4")
    print("=" * 70)
    ...
    omni_df.to_csv(os.path.join(FIG_ROOT_70B, "omnibus_interactions.csv"), index=False)
    _bs.write_omnibus_tex(omni_df, os.path.join(FIG_ROOT_70B, "omnibus_interactions.tex"))
    ...
    mod_df.to_csv(os.path.join(FIG_ROOT_70B, "initial_level_moderation.csv"), index=False)
    _bs.write_moderation_tex(mod_df, os.path.join(FIG_ROOT_70B, "initial_level_moderation.tex"))
    ...
    disp_df.to_csv(os.path.join(FIG_ROOT_70B, "dispersion.csv"), index=False)
    _bs.write_dispersion_tex(disp_df, os.path.join(FIG_ROOT_70B, "dispersion.tex"))

    print("\n" + "=" * 70)
    print(f"All outputs saved to: {FIG_ROOT_70B}")
