...
**To run just the 5 headline main-paper analyses** (contribution trajectories, pairwise OLS+Wald, perceptual convergence, gap-based alignment, social-selection mechanism) with exact commands and CLI flags, see `aaai_code_release/README.md` — it's the paper's own reproducibility writeup and stays current with `code/analysis/` as of the 2026-08-27 merge (see the update note in §0).
...
**Correction (2026-08-27):** despite the "archived to `archive/...`" wording above and in the 2026-08-24 note below, **no `archive/` directory exists anywhere in this repo on disk** (checked directly — zero files under any `archive/` path, at any depth). Whatever `git mv` originally staged into those gitignored paths was apparently deleted from disk afterward without being unstaged; committing that state (2026-08-27) finalized it as a plain removal. The named files are **not lost** — every one is recoverable via `git show <pre-2026-08-27 commit>:<original path>` (e.g. `git show ccf57d1:code/analysis/sobel_mediation.py`) — but they are not sitting in a browsable local archive folder the way this section implies. Treat "archived to archive/" throughout this doc as "removed from the tree, recoverable from git history only."

**Update (2026-08-27):** merged `aaai_code_release/` (the AAAI paper's curated, polished code-release subset) back into `code/analysis/`/`code/experiments/` so there is one working copy, not two. `aaai_code_release/` itself was kept (per author's request — useful as the frozen snapshot actually linked from the paper), but is no longer the "improved" branch: its upgrades (better docs/comments, portable `BASE` paths, two real path-resolution bugfixes) are now in `code/analysis/` too. Specifics:
- Direct content sync (cosmetic/portability only, no logic change): `model_specs.py`, `behavioral/behavior_quantified.py`, `behavioral/behavior_quantified_noexpect.py`, `behavioral/contribution_all_conditions_plot.py`, `behavioral/community_group_contribution_tables.py`, `perception/perception_consensus.py`, `alignment/gap_based_alignment.py`, `alignment/lagged_alignment_check.py`, `selection/community_ss_mechanism_table.py`, `selection/social_selection_feedback_analysis.py`, `experiments/SoNoLiSi_v5_local.py`, `experiments/new_group_IN_SoNoLiSi_os_local.py` (the latter two: kept `CACHE_DIR` defaulting to this machine's `/data3/models/hub` rather than adopting aaai's portable-but-wrong-here `~/.cache/huggingface/hub` default).
- **`build_agent_round_panel.py` moved from `selection/` to `alignment/`** (matches aaai's reorg — it has no functional tie to `selection/`; only `gap_based_alignment.py` reads its output). All 6 importers use the shared anchor-walk `sys.path` pattern, so nothing besides the file's own location needed to change; verified by simulating the anchor-walk from each dependent.
- **`analyze_structural_robustness.py` moved from `figures/SUPPLEMENTARY_RESULTS/5_community_group_mcpr/community_group_robusntess/` into `code/analysis/behavioral/`** — it's source code, it was living in an output/figures directory. `community_group_contribution_tables.py` and `alignment/community_group_perception_action_tables.py` (its two importers) both had a hardcoded fallback `sys.path` insert pointing at the old figures/ location; the former's was already dropped in aaai's version (co-location made it redundant), the latter's was removed by hand here for the same reason.
- **Fixed, not just copied**: `selection/social_selection_feedback_analysis_noexpect.py`'s `OUT_DIR` used a bare `dirname(__file__)` in aaai's version, which resolves one level too deep (`.../selection/exports/...` instead of the documented flat `code/analysis/exports/social_selection_feedback_noexpect/`) — this was a latent bug already present in aaai_code_release's own copy (its sibling `social_selection_feedback_analysis.py` uses the correct `_ANALYSIS_DIR` anchor-walk; the noexpect script didn't). Fixed in both `code/analysis/` and `aaai_code_release/` to use the same anchor pattern.
- 3 files (`community_group_perception_action_tables.py`, `gap_convergence_did.py`, `perception_action_gap_plot.py`, plus everything else in `code/analysis/`) have no aaai_code_release counterpart and were untouched — aaai_code_release only ever covered 5 headline main-paper analyses, not the supplementary/robustness/discussion-mechanism/MCPR work that makes up most of `code/analysis/`.
